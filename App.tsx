...
const WhatsAppButton: React.FC = () => (
  <a href="https://wa.me/420605034911" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[100] flex items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-[#25D366] text-white rounded-full shadow-2xl hover:scale-110 transition-transform">
...
const Layout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#050505] text-white">
      <SEOManager />
      <header className={`fixed top-0 left-0 w-full z-[80] p-6 md:p-10 flex justify-between items-center transition-all duration-[1000ms] cubic-bezier(0.16, 1, 0.3, 1) ${showHeader || isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'} reveal-down`}>
        <button onClick={() => setIsMenuOpen(true)} className="group flex items-center space-x-3 focus:outline-none z-50">
          <div className="flex flex-col items-start space-y-1.5">
            <span className="w-8 h-[1.2px] bg-white group-hover:bg-[#dbad1e] transition-all"></span>
            <span className="w-5 h-[1.2px] bg-white group-hover:bg-[#dbad1e] transition-all"></span>
          </div>
        ...
        <button onClick={() => setIsSearchOpen(true)} className="group flex items-center h-10 w-10 justify-center bg-white/0 hover:bg-white/5 rounded-full transition-all z-50">
          <svg className="w-4 h-4 md:w-5 md:h-5 text-white/40 group-hover:text-white transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        </button>
      </header>
      ...
      <aside className={`fixed top-0 left-0 h-full w-full md:w-[450px] bg-black z-[110] transition-transform duration-700 p-8 md:p-14 flex flex-col border-r border-white/5 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <button onClick={() => setIsMenuOpen(false)} className="mb-10 flex items-center space-x-4 opacity-30 hover:opacity-100 transition-opacity">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M6 18L18 6M6 6l12 12" /></svg>
          <span className="text-[9px] font-bold uppercase tracking-widest">ZAVŘÍT</span>
        </button>
        <nav className="flex flex-col space-y-4 overflow-y-auto pr-4 custom-scrollbar flex-grow">
          {menuLinks.map((link) => (
            <Link key={link.to} to={link.to} onClick={() => setIsMenuOpen(false)} className="text-xl md:text-2xl font-bold uppercase font-heading hover:text-[#dbad1e] transition-all tracking-tight">
              {link.label}
            </Link>
          ))}
        </nav>
        <div className="pt-10 flex items-center space-x-6 border-t border-white/5 mt-auto">
          <a href="https://www.instagram.com/bbcars_4you/" target="_blank" className="text-white/20 hover:text-white transition-colors group">
          ...
          <a href="https://www.facebook.com/profile.php?id=100030982372648" target="_blank" className="text-white/20 hover:text-white transition-colors group">
